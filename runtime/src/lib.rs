...
impl KnownAtoms {
    ...
    fn nil_() -> Rc<str> {
        Self::with(|atoms| atoms.nil_.clone())
    }
}
...
impl Any {
    ...
    fn iter(&self) -> Result<impl IntoIterator<Item = Any>, RuntimeError> {
        struct ObjectIter {
...
pub struct Runtime {
    pub global: BTreeMap<String, Any>,
    pub local: Option<BTreeMap<String, Any>>,
...
impl Runtime {
                    ...
                    Any::Object(h) => {
                        ...
                        match &borrowed.kind {
                            ObjectKind::Record(map) => map
                                ...
                                .cloned()
                                .ok_or(RuntimeError::FieldError(field_ident.name().to_owned())),
                            _ => Err(RuntimeError::FieldError(field_ident.name().to_owned())),
                        }
                    }
                    _ => Err(RuntimeError::FieldError(field_ident.name().to_owned())),
                }
                        ...
                        match &mut borrowed.kind {
                            ObjectKind::List(vec) => {
                                ...
                                let ui = usize::try_from(i).map_err(|_| {
                                    RuntimeError::IndexError(format!("invalid index: {}", i))
                                })?;
                                if ui >= vec.len() {
                                    return Err(RuntimeError::IndexError(format!(
                                        "out of bounds: {}",
    ...
    fn bind_pattern(&mut self, pat: &Pat, val: &Any) -> Result<(), RuntimeError> {
        match pat.kind() {
            PatKind::Ident(id) => {
                self.set_var(id.name(), val.clone());
                Ok(())
            }
            ...
            PatKind::Literal(lit) => {
                ...
                match (lit, val) {
                    (Literal::Number(nlit), Any::Number(Number::Integer(i))) => {
                    ...
                    (Literal::Number(nlit), Any::Number(Number::Float(f))) => {
                        let repr = nlit.value();
                        if let Ok(parsed) = repr.parse::<f64>() {
                            if (parsed - *f).abs() < std::f64::EPSILON {
                                return Ok(());
                            }
                        }
                        Err(RuntimeError::Other("pattern match failed".into()))
                    }
                    (Literal::String(slit), Any::String(s)) => {
                        if slit.unescape() == &**s {
                            Ok(())
                        } else {
                            Err(RuntimeError::Other("pattern match failed".into()))
                    ...
                    ObjectKind::Record(map) => {
                        for f in fields.iter() {
                            if let Some(v) = map.get(f.key().name()) {
                                match f.pat() {
                                    None => {
                                        self.set_var(f.key().name(), v.clone());
                                    }
                                    Some(pat) => {
...
fn eval_unary(op: &UnaryOp, a: &Any) -> Result<Any, RuntimeError> {
    use raft_ast::UnaryOpKind::*;
    match op.kind() {
        Not => Ok(a.not()),
        BitNot => a.bit_not(),
        Pos => a.pos(),
...
fn eval_binary(op: &BinaryOp, a: &Any, b: &Any) -> Result<Any, RuntimeError> {
    use raft_ast::BinaryOpKind::*;
    match op.kind() {
        BitAnd => a.bit_and(b),
        BitOr => a.bit_or(b),
        BitXor => a.bit_xor(b),
...
mod tests {
    ...
    fn while_else_not_on_break_or_return() {
        ...
        let stmts = ast_from_str(src);
        let mut rt = Runtime::new();
        let _ = rt.exec_block(&stmts).unwrap();
