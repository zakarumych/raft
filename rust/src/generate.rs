...
impl BundleGenerator {
    ...
    pub fn generate_lib_rs(&self) -> String {
        ...
        let _ = writeln!(
            src,
            "    let mut host = unsafe {{ Host::from_raw(ffi_host.raw) }};"
        );
        let _ = writeln!(src, "    let mut modules: Vec<(RcStr, Val)> = Vec::new();");
        for (name, _) in &self.modules {
            let _ = writeln!(
                src,
                "    modules.push((RcStr::new({name:?}), {name}::load(&mut host)?));"
            );
        }
        let _ = writeln!(src, "    bundle.modules = Val::record(modules).into_ffi();");
        let _ = writeln!(src, "    Ok::<(), RuntimeError>(())");
        let _ = writeln!(src, "}});");
        src
    }
